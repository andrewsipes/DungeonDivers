...
private:
	...
	struct MODEL_ENTRY
	{
		...
		GW::MATH::GOBBF ComputeOBB() const {
			...
			out.center.x = (boundary[0].x + boundary[4].x) * 0.5f;
			out.center.y = (boundary[0].y + boundary[1].y) * 0.5f;
			out.center.z = (boundary[0].z + boundary[2].z) * 0.5f;
			out.extent.x = std::fabsf(boundary[0].x - boundary[4].x) * 0.5f;
			out.extent.y = std::fabsf(boundary[0].y - boundary[1].y) * 0.5f;
			out.extent.z = std::fabsf(boundary[0].z - boundary[2].z) * 0.5f;
			return out;
		}
	};
