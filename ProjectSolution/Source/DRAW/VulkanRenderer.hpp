...
#pragma endregion
	...
	inline void Construct_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		...
		UTIL::matrixProxy.ProjectionVulkanLHF(G2D_DEGREE_TO_RADIAN_F(initializationData.fovDegrees), aspectRatio, initializationData.nearPlane, initializationData.farPlane, vulkanRenderer.projMatrix);
	...
	inline void Update_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		auto& vulkanRenderer = registry.get<VulkanRenderer>(entity);
		...
		if (registry.all_of< VulkanVertexBuffer, VulkanIndexBuffer>(entity))
		{
			...
			auto& instances = registry.group<GeometryData>(entt::get<GPUInstance>);
			...
			for (auto ent : instances) {
				...
				auto& gpuInst = instances.get<GPUInstance>(ent);
				auto& geo = instances.get<GeometryData>(ent);
				bufferList.push_back(gpuInst);
				if (drawInstructions.find(geo) == drawInstructions.end())
					drawInstructions[geo] = 1;
				else
					drawInstructions[geo] += 1;
			}
			registry.emplace<std::vector<GPUInstance>>(entity, bufferList);
			...
			{
				vkCmdDrawIndexed(commandBuffer, draw.first.indexCount, draw.second, draw.first.indexStart, draw.first.vertexStart, instanceCounter);
				instanceCounter += draw.second;
			}
		}
	...
	// run this code when a VulkanRenderer component is updated
	inline void Destroy_VulkanRenderer(entt::registry& registry, entt::entity entity)
	{
		auto& vulkanRenderer = registry.get<VulkanRenderer>(entity);
